...
7. **Three novel PTO instructions enabled by the v2 pipeline (§7.5):**
...
### 3.1 Ports (unchanged from [`vector4k.md`](vector4k.md))
...
Ports attach to the 8R/8W TRegFile-4K ([`tregfile4k.md`](tregfile4k.md) §3). VEC-4K-v2 uses **two** of the eight physical read ports (by convention **R0 as Port A** and **R4 as Port B**, same binding as [`vector4k.md`](vector4k.md) §4.4) and **two** of the eight physical write ports (**W0** / **W4**).
...
**Operand roles.** The three ISA-level tile operands have distinct payload semantics and distinct fetch policies:

| Operand | Role | Payload | Fetched when | Storage |
|---------|------|---------|--------------|---------|
| **A** | Value tile (primary) | `R × C × E` bytes of tile data | Always | `SA` (§4.2) |
| **B** | Value tile (secondary, optional) | `R × C × E` bytes of tile data | When the op is binary (`N_val = 2`) | `SB` (§4.2) |
| **C** | **Per-element bitmask (predicate)** — 1 bit per logical element of `A` / `B`, selecting which lanes participate in the ALU op (§3.3c) | `⌈R · C / 8⌉` bytes of packed mask bits | **Only** when the instruction needs a mask (`has_mask = 1` in `SOP`, §4.4); **skipped entirely** for full-tile ops | `SC` (§4.2.2) |

When `C` is not fetched, the stage (B) ALU's mask input defaults to `IMM_ALL_ONES` (§5.8) and every lane participates unconditionally. Instructions that need a third *value* tile (not a mask) are not supported; ternary `TFMA` uses the accumulator feedback (§5.5, §5.8) as its third value input instead.
...
| **C**   | Port A **or** Port B (whichever is free during the second fetch epoch, §6.3) | `SC` | TRegFile-4K col-mode read on whichever port fetches C |
...
#### 3.3c Operand `C` = per-element bitmask

The `C` tile operand is always a **packed per-element bitmask**: one bit per logical element of the ALU's operating format (FP32 → 1 bit per 32-bit lane, FP16 → 1 bit per 16-bit lane, FP8 → 1 bit per 8-bit lane, FP4 → 1 bit per 4-bit lane). Layout is row-major over the `R × C` element grid of the associated value tile:

```
  mask_bit(r, c) = SC[ (r · C + c) / 8 ].bit[ (r · C + c) % 8 ]
```

Total mask size is `R · C / 8` bytes. For a 4 KB FP32 tile this is 1024 elements → 128 B (well within a single strip); for FP16 it is 256 B; for FP4 it is 1024 B (2 strips). The `SC` staging register is still 4 KB to match `SA` / `SB` (§4.2); unused strips stay idle but no extra port bandwidth is charged.

**Fetch policy.** The issue packet carries a `has_mask` bit. When `has_mask = 0` the mask tile is **not read** — no TRegFile epoch is consumed for `C`, and the stage (B) mask input is tied to `IMM_ALL_ONES` statically by the microcode assembler. When `has_mask = 1`, microcode fetches `C` from TRegFile, short-circuiting to cover only the strips actually needed (`⌈R · C / (8 · 512)⌉`), trading one full 8-cycle epoch for as few as 1–2 strip cycles that piggyback on a value-tile epoch (§6.2, §6.3).

**Transpose.** The `C` mask, conceptually, has the same logical `(r, c)` layout as the value tile; `is_transpose_C` and `tilelet_xpose` of the `SC` read port transpose the mask the same way they transpose a value tile, so masked-transpose ops stay consistent without extra handling.
...
| `N_val` | `is_transpose` mix | `has_mask` | `N_epoch` | Best-case `T_fetch` (aligned) | Worst-case `T_fetch` (misaligned) |
|--------:|--------------------|-----------:|----------:|-------------------------------:|----------------------------------:|
| 1 | any         | 0 | 1 | **8 cy**  | 15 cy |
| 1 | any         | 1 | 1 | **8 cy** (mask rides the idle port in epoch 0) | 15 cy |
| 2 | uniform     | 0 | 1 | **8 cy**  | 15 cy |
| 2 | uniform     | 1 | 1 | **8 cy** (mask rides the idle port in epoch 0) | 15 cy |
| 2 | mixed       | 0 | 2 | **16 cy** (`xpose_mismatch`) | 23 cy |
| 2 | mixed       | 1 | 2 | **16 cy** (mask rides the idle port in epoch 0 or 1) | 23 cy |
...
`TADD`, `TMUL`, `TAND`, `TCMP`, `TCVT`, predicated / masked variants of each of the above (gated by `C`, §3.3c), and **ternary-via-accumulator** ops (`TFMA_ACC`, `TSELECT`-over-Acc):
...
|------|-------------|---------------|------------|
| Tile operands per instruction | 1–2 (all value tiles) | **1–2 value tiles (`A`, `B`) + optional 1-bit-per-element bitmask tile (`C`)** | Per-element predication / masking on every op (masked `TADD` / `TMUL` / `TMAX` / …, `TSELECT`, masked reductions); full-tile ops skip `C` fetch entirely (§3.3c, §6.2). Ternary FMA uses `ACC_READ_*` as the 3rd value input via the Stage (B) input MUX (§5.8), not a third value tile |
| Tile results per instruction | 1 | **1–2** | `TSORT32`, `TROWARGMAX`, `TDIVMOD` emit value+index in a single op |
| Read ports | 2 × 512 B | 2 × 512 B | unchanged |
...
- [`vector4k.md`](vector4k.md) — VEC-4K v1 (the direct predecessor). §3–§9 of this document inherit all compute-stage formulas and templates from v1.
